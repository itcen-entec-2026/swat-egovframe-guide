    ...
    public void run(String... args) throws Exception {
		...
		if (jobPath == null || jobIdentifier == null) {
			...
			if (jobIdentifier == null) {
				jobIdentifier = "jdbcToJdbcJob";
			}
			
			String message = "Using default values:\n" +
							"JobPath: " + jobPath + "\n" +
							"JobIdentifier: " + jobIdentifier + "\n" +
							"To specify custom values, use: java -jar db.commandLine.jar <jobPath> <jobIdentifier> [parameters...]\n" +
							"Available jobs: jdbcToJdbcJob, mybatisToDelimitedJob, mybatisToFixedLengthJob, mybatisToMybatisJob";
			LOGGER.info(message);
		...
		// timestamp 파라미터 추가
		List<String> finalParams = new ArrayList<String>(Arrays.asList(parameters));
		finalParams.add("timestamp=" + new Date().getTime());
		String[] finalParameters = finalParams.toArray(new String[finalParams.size()]);

		int result = command.start(jobPath, jobIdentifier, finalParameters, opts);
		command.exit(result);

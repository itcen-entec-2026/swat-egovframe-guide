 ...
 *  </pre>
 */
public class CustomerIncreaseProcessor2 implements ItemProcessor<Customer, Customer2> {

	private static final Logger LOGGER = LoggerFactory.getLogger(CustomerIncreaseProcessor2.class);
	// 증가할 수
	public static final BigDecimal FIXED_AMOUNT = new BigDecimal("10");
	...
	public Customer2 process(Customer item) throws Exception {
		...
		Customer2 item2 = new Customer2();
		BeanUtils.copyProperties(item, item2);
		
		item2.setCredit(item.getCredit().add(FIXED_AMOUNT).add(new BigDecimal(item.getId())));
		item2.setId(item.getId()+(ID_INCREASE_STEP*(this.jobId+1)));
